...
Actions.prototype.init = function () {
	...
	this.addAction('exportAs', function () { ui.save(); }, null, null, Editor.ctrlKey + '+Shift+S').isEnabled = isGraphEnabled;
	...
	this.addAction('importFromFile', function () {
		importText().then(xml => {
			// console.log(xml);
			// 匹配 <mxGraphModel> 标签
			var regex = /<mxGraphModel[^>]*>([\s\S]*?)<\/mxGraphModel>/g;
			var matches = [];
			var match;
			while (match = regex.exec(xml)) {
				var mxGraphModelTag = match[0]; // 完整的 <mxGraphModel> 标签
				var attributes = mxGraphModelTag.match(/(\S+)=["']?((?:.(?!["']?\s+(?:\S+)=|[>"']))+.)["']?/g); // 属性列表
				var content = match[1]; // 标签内的内容
				matches.push({ mxGraphModelTag, attributes, content });
			}
			var data = ui.editor.graph.zapGremlins(mxUtils.trim(matches[0].mxGraphModelTag));
