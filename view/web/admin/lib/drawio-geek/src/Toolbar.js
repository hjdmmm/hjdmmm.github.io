...
Toolbar.prototype.init = function () {
...

	{
		...
		cancelButton.className = 'toolbarButton';
		cancelButton.style.backgroundColor = 'rgb(255,136,0)';
		cancelButton.title = '不保存退出';
		cancelButton.innerText = '不保存退出';

		mxEvent.addListener(cancelButton, 'click', mxUtils.bind(this, function() {
		...
		saveButton.style.backgroundColor = 'rgb(0, 102, 255)';
		saveButton.title = '保存并退出';
		saveButton.innerText = '保存并退出';

		mxEvent.addListener(saveButton, 'click', mxUtils.bind(this, function() {
			if (!mxUtils.confirm(mxResources.get('areYouSure'))) {
				return;
			}
			...
			const border = "5";
			try {
				this.editorUi.exportToCanvas(mxUtils.bind(this, function(canvas) {
						try {
							const name = urlParams['name'] ? decodeURIComponent(urlParams['name']) : "Unknown";

							let sourceFile;
							{
								const xml = mxUtils.getXml(editorUIInstance.editor.getGraphXml());
								sourceFile = new File([new Blob([xml], { type: 'text/xml' })], name, { type: 'text/xml' });
							}

							let pictureFile;
							{
								const fileData = this.editorUi.getFileData(true, null,
									null, null, ignoreSelection, false, null, null, null,
									null, null, scale, border);
								const png = this.editorUi.createImageDataUri(canvas, fileData, "png", null);
								const parts = png.split(';base64,');
								const contentType = parts[0].split(':')[1];
								pictureFile = new File([this.editorUi.base64ToBlob(parts[1], contentType)], name, { type: contentType });
							}
							...
							if (urlParams['id']) {
								...
								ajax.post("admin/drawio/updateFile", formData, mxUtils.bind(this, function () {
									this.editorUi.editor.graph.model.endUpdate();
									window.history.back();
								}), mxUtils.bind(this, function () {
									mxUtils.alert(mxResources.get('unknownError'));
									this.editorUi.editor.graph.model.endUpdate();
								}));
							} else {
								const formData = new FormData();
								...
								ajax.post("admin/drawio", formData, mxUtils.bind(this, function () {
									this.editorUi.editor.graph.model.endUpdate();
									window.history.back();
								}), mxUtils.bind(this, function () {
									mxUtils.alert(mxResources.get('unknownError'));
					...
					}), null, this.editorUi.thumbImageCache, null, mxUtils.bind(this, function(e) {
						this.editorUi.handleError(e);
					}), null, ignoreSelection, scale || 1, false, false,
					null, null, border, true, false, "light", "diagram");
			} catch (e) {
				this.editorUi.handleError(e);
			}
		}));
