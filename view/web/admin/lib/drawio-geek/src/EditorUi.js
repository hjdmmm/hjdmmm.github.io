...
EditorUi.prototype.open = function () {
	...
	try {
		if (urlParams['id']) {
			...
			ajax.get("admin/drawio/" + urlParams['id'], null, mxUtils.bind(this, function (vo) {
				urlParams['name'] = encodeURIComponent(vo.name);

				const regex = /<mxGraphModel[^>]*>([\s\S]*?)<\/mxGraphModel>/g;
				const matches = [];
				let match;
				while (match = regex.exec(vo.source)) {
					const mxGraphModelTag = match[0]; // 完整的 <mxGraphModel> 标签
					const attributes = mxGraphModelTag.match(/(\S+)=["']?((?:.(?!["']?\s+(?:\S+)=|[>"']))+.)["']?/g); // 属性列表
					const content = match[1]; // 标签内的内容
					matches.push({ mxGraphModelTag, attributes, content });
				}
				const data = this.editor.graph.zapGremlins(mxUtils.trim(matches[0].mxGraphModelTag));
...
/**
 * 将当前Png保存在给定的文件名下，导出为png 的逻辑实现。
 */
EditorUi.prototype.savePng = function () {
	var dlg = new FilenameDialog(this, this.editor.getOrCreateFilename({ suffix: "png" }), mxResources.get('save'), mxUtils.bind(this, function (name) {
...
EditorUi.prototype.saveXml = function () {
	var dlg = new FilenameDialog(this, this.editor.getOrCreateFilename({ suffix: "xml" }), mxResources.get('save'), mxUtils.bind(this, function (name) {
